...
from countries.serializers import CountrisSelializer
...
def countries_list(request):
    if request.method == 'GET':
        ...
        countries_serializer = CountrisSelializer(countries, many=True)
        return JsonResponse(countries_serializer.data, safe=False)

    elif request.method == 'POST':
        countries_data = JSONParser().parse(request)
        countries_serializer = CountrisSelializer(data=countries_data)
...
def countries_details(request, pk):
    ...
    if request.method == 'GET':
        countries_serializer = CountrisSelializer(countries)
        return JsonResponse(countries_serializer.data)
    elif request.method == 'PUT':
        countries_data = JSONParser().parse(request)
        countries_serializer = CountrisSelializer(countries, data=countries_data)
        if countries_serializer.is_valid():
            countries_serializer.save()
            return JsonResponse(countries_serializer.data)
        return JsonResponse(countries_serializer.errors, status=status.status.HTTP_400_BAD_REQUEST)
